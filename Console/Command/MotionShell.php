...
class MotionShell extends AppShell {
    ...
    public function matchParliamentarian() {
        ...
        foreach ($this->motionIdStack AS $motionId => $motionSummary) {
            ...
            $toMatchString = is_array($motionSummary['detail']['提案單位/人']) ? implode(",", $motionSummary['detail']['提案單位/人']) : $motionSummary['detail']['提案單位/人'];
            $toMatchString .= is_array($motionSummary['detail']['連署人']) ? implode(",", $motionSummary['detail']['連署人']) : $motionSummary['detail']['連署人'];
            foreach ($parliamentarians AS $parliamentarianId => $parliamentarian) {
